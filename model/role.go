...
type Role struct {
	...
	Status  uint    `gorm:"type:tinyint(1);default:1" json:"status"`
	Sort    uint    `gorm:"type:tinyint(1);default:1;comment:'角色排序(排序越大权限越低, 不能查看比自己序号小的角色, 不能编辑同序号用户权限, 排序为1表示超级管理员)'" json:"sort"`
